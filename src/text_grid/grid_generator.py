...
async def generate_biome(theme_prompt, structure_type_list):
    ...
    try:
        ...
        # --- NEW DEBUG PRINT ---
        logger.debug(f"DEBUG: Layout *after* generate_grid_from_hints and *before* internal check: {layout}")
        
        if not layout or not any(0 not in row for row in layout): 
            logger.warning("Generated layout is empty or contains no structures. Check grid_placement_logic or LLM hints.")
        ...
        logger.error(f"[ERROR] Failed to generate grid from hints: {e}")
        grid_width = llm_hints["grid_dimensions"]["width"] if "grid_dimensions" in llm_hints else 10
        grid_height = llm_hints["grid_dimensions"]["height"] if "grid_dimensions" in llm_hints else 10
        layout = [[0 for _ in range(grid_width)] for _ in range(grid_height)]
