...
STAGE1_DEFAULTS: dict = {
    "denoise":           0.65,   # high — needed to actually change pose
    "cfg":               7.0,
    "steps":             25,
    "openpose_weight":   1.00,   # strong T-pose lock
    "canny_weight":      0.25,   # light structural hint
    "ip_adapter_weight": 0.65,   # identity from Flux
    "category":          "humanoid",
}
...
def _load_openpose_ref(path: str) -> Image.Image:
    ...
    if path and os.path.isfile(path):
        try:
            ...
            logger.info(f"[sd] T-pose reference loaded: {path}")
            return img
