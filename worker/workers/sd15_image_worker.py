...
# Same ControlNet used for both bipedal and quadruped
# (quad_animalpose.png skeleton image provides the pose guidance)
CONTROLNET_ANIMAL_ID   = CONTROLNET_OPENPOSE_ID
...
class SD15Worker(BaseWorker):
    ...
    def load_models(self):
        self.logger.info(f"Loading ControlNet from local: {CONTROLNET_OPENPOSE_ID}")
        self.controlnet_openpose = ControlNetModel.from_pretrained(
            CONTROLNET_OPENPOSE_ID,
            torch_dtype=torch.float16,
            local_files_only=True,
        ).to("cuda")
        # Same model for quadrupeds (quad skeleton image provides pose guidance)
